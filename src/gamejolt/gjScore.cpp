...
// ****************************************************************
/*  */   
int gjScore::__GetRankCallback(const std::string& sData, void* pAdd, int* piOutput)
{
    // parse output
    gjDataList aaReturn;
    if(m_pAPI->ParseRequestKeypair(sData, &aaReturn) != GJ_OK)
    {
        gjAPI::ErrorLogAdd("API Error: could not parse score");
        return GJ_REQUEST_FAILED;
    }



    if(piOutput) (*piOutput) = atoi(SAFE_MAP_GET(aaReturn[0], "rank").c_str());
    return GJ_OK;
}
