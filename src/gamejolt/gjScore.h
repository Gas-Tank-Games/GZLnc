...
public:
    ...
    //! @}
                          ...
                          inline int GetRankNow(int* piOutput)           {return this->__GetRank(piOutput, GJ_NETWORK_NULL_API(int));}
    template <typename T> inline int GetRankCall(GJ_NETWORK_OUTPUT(int)) {return this->__GetRank(NULL, GJ_NETWORK_OUTPUT_FW);}
...
private:
    ...
    /*! \name Superior Request Functions */
    //! @{
    template <typename T> int __GetRank(int* piOutput, GJ_NETWORK_OUTPUT(int));
    //! @}

    /*! \name Callback Functions */
    //! @{
    int __GetRankCallback(const std::string& sData, void* pAdd, int* piOutput);
    //! @}
};
...
// ****************************************************************
/*  */
template <typename T> int gjScore::__GetRank(int* piOutput, GJ_NETWORK_OUTPUT(int))
{
    //if(m_iID <= 0) return GJ_INVALID_CALL;

    const bool bNow = piOutput ? true : false;

    // send get user request
    std::string sResponse;
    if(m_pAPI->SendRequest("/scores/get-rank/"
                           "?game_id="  + m_pAPI->GetProcGameID()                        +
                           "&table_id=" + gjAPI::UtilIntToString(m_pScoreTable->GetID()) +
                           "&sort="     + gjAPI::UtilIntToString(m_iSort),
                           bNow ? &sResponse : NULL, this, &gjScore::__GetRankCallback, NULL, GJ_NETWORK_OUTPUT_FW)) return GJ_REQUEST_FAILED;

    if(bNow) return this->__GetRankCallback(sResponse, NULL, piOutput);
    return GJ_OK;
}
